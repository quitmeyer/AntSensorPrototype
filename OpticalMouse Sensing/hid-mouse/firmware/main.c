 ...
 * Edited by Ryan Owens to use the ADNS2620 evaluation board
...
/* 	SparkFun Modifications
 *	Makefile was changed to compile for the AtMega328 instead of the AtMega168
 *	Same pinout used
 *  Functions added to the main.c file for communicating with the ADNS2610 Mouse Sensor
 *	Uses pins PC0 and PC1 for I2C-like interface to the ADNS2610 module
*/
...
/* SFE Capture Mouse Function */
static void getMouseMovement(void)
{
	reportBuffer.buttonMask = ((!(PINB & 0x01)) << 1) | (!(PINB & 0x02));
...
{
    char value=0;
	sbi(ADNS_DIR, ADNS_SDIO);	//Make sure the SDIO pin is set as an output.
    sbi(ADNS_PORT, ADNS_SCL); //Make sure the clock is high.
    ...
    for(address_bit=7; address_bit >=0; address_bit--){
        cbi(ADNS_PORT, ADNS_SCL);  //Lower the clock
        sbi(ADNS_DIR, ADNS_SDIO); //Make sure the SDIO pin is set as an output.
    ...
    _delay_us(120);   //Allow extra time for ADNS2610 to transition the SDIO pin (per datasheet)
    //Make SDIO an input on the microcontroller
    cbi(ADNS_DIR, ADNS_SDIO);	//Make sure the SDIO pin is set as an input.
	sbi(ADNS_PORT, ADNS_SDIO);	//Enable the pull-up on the SDIO pin.
        
	int value_bit=0;
    //Send the Value byte to the ADNS2610
    for(value_bit=7; value_bit >= 0; value_bit--){
        cbi(ADNS_PORT, ADNS_SCL);  //Lower the clock
        _delay_us(1); //Allow the ADNS2610 to configure the SDIO pin
        sbi(ADNS_PORT, ADNS_SCL);  //Raise the clock
        _delay_us(1);
...
//usage: write(CONFIGURATION_REGISTER, config_setting);
 void write(char address, char value)
 {
    ...
    for(address_bit=7; address_bit >=0; address_bit--){
        ...
        _delay_us(1); //Give a small delay (only needed for the first iteration to ensure that the ADNS2610 relinquishes
    ...
    for(value_bit=7; value_bit >= 0; value_bit--){
        cbi(ADNS_PORT, ADNS_SCL);  //Lower the clock
        //If the current bit is a 1, set the SDIO pin. If not, clear the SDIO pin
        if(value & (1<<value_bit))sbi(ADNS_PORT, ADNS_SDIO);
        else cbi(ADNS_PORT, ADNS_SDIO);
        _delay_us(1);
        sbi(ADNS_PORT, ADNS_SCL);
        _delay_us(1);
...
void setupAdns(void)
{
	sbi(ADNS_DIR, ADNS_SCL);	//Set the clock as an output
	sbi(ADNS_DIR, ADNS_SDIO); 	//Set the SDIO as an output...for now

    sbi(ADNS_PORT, ADNS_SCL);  //Set the Clock High
    sbi(ADNS_PORT, ADNS_SDIO);       //Set the SDIO pin high
 }
